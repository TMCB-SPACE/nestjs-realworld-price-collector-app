...
`;
  ...
  options
    .addServer(`http://${configService.get('api.host')}:${configService.get('api.port')}`, 'Local')
    .setTitle(`nestjs-realworld-price-collector-app`)
  ...
  await app.listen(configService.get('api.port') ?? '3002', configService.get('api.host'));
}
